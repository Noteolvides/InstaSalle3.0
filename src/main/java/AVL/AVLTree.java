        ...
        if (node == null) {
            ...
            if (cmp > 0) {//Vamos moviendonos de derecha e izquierda por las ramas
                ...
                if (act.bool) { //En el caso de que menos insertado un nodo o una actualizacion
                    if (node.factor == +1) {//Si es la primera vez que se inserta como nos habiamos movido por la rama de izquierda lo ponemos a -1
                        node.factor = 0;//La rama esta balanceada :) que bien
                        act.bool = false;// Ya no tenemos que hacer nada mas
                    } else if (node.factor == 0) {//Si al regresar por la recursividad el ya tenia un nodo en la derecha
                        node.factor = -1;
                        act.bool = true;//Decimos que queremos decir que actualizen los indeces
                    } else if (node.factor == -1) { //Si al regresar por la recusividad el ya tenia un nodo en la izquieda es mal asunto tenemos que rotar
                        if (node.leftChild.factor == -1) {//Primero tenemos que ver que tipo de rotacion es
                            node = rotateLL(node);
                        } else { //En el caso que mi hijo izquiero ya tiene un -1 es una rotacion a la izquierda
                            node = rotateLR(node);//En el caso que tenga un +1 rotacion LR
                        }
                        act.bool = false;//Limpiamos el booleano
                    }
                }
            }
            if (cmp < 0) {
                ...
                if (act.bool) {
                    if (node.factor == -1) {//Si el tenia un hermano pues ya esta equilibrado
                        node.factor = 0;
                        act.bool = false;
                    } else if (node.factor == 0) {//En el la primera iteracion cambiamos el factor al poner un nodo en la derecha ponemos a +1
                        node.factor = +1;
                        act.bool = true;//Todavia no hemos terminao de ver si esta equilibrado faltaria una iteracion
                    } else if (node.factor == +1) {//En el caso que no tenga +1 en la 3 subida tenemos que actulizar
                        if (node.rightChild.factor == +1) {//si mi hijo tiene un nodo con factor 3 eso es que ha sido un inserccion RR
                            node = rotateRR(node);
                        } else {
                            node = rotateRL(node);//En el caso contrario es una inserccion RL
                        }
                        act.bool = false;
                    }
                }
